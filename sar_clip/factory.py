...
from .transform import AugmentationCfg, PreprocessCfg, merge_preprocess_dict, merge_preprocess_kwargs
from .tokenizer import SimpleTokenizer, CustomCLIPTokenizer
...
def get_tokenizer(
    ...
    tokenizer_kwargs = text_config.get('tokenizer_kwargs', {})
    tokenizer_kwargs.update(kwargs)

    if context_length is None:
        context_length = text_config.get('context_length', 77)  # Default value is 77. It can be adjusted as needed.

    # **Change to using the local tokenizer**
    tokenizer_path = cache_dir if cache_dir else "./"

    vocab_path = os.path.join(tokenizer_path, "vocab.json")
    merges_path = os.path.join(tokenizer_path, "merges.txt")
    if os.path.exists(vocab_path) and os.path.exists(merges_path):
        tokenizer = CustomCLIPTokenizer(
...
def load_checkpoint(
    ...
    if Path(checkpoint_path).suffix in ('.npz', '.npy'):
        ...
        from sar_clip.convert import load_big_vision_weights
        load_big_vision_weights(model, checkpoint_path)
        return {}

    state_dict = load_state_dict(checkpoint_path, device=device, weights_only=weights_only)

    # Detect & convert 3rd party state_dicts -> sar_clip
