...
class SimpleTokenizer(object):
    ...
    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None):
        ...
        #        print(f"⚠️  The {i + 1}th sentence exceeds {context_length} tokens (including {len(tokens)} tokens): \n{texts[i]}")
        #        print(f"➡️  Token sequence: {tokens}\n")
...
class CustomCLIPTokenizer(CLIPTokenizer):
    def __init__(
        ...
        # ** Compatible with the functions of SimpleTokenizer **
        self.context_length = context_length  # Set the maximum context length
        self.sot_token_id = self.convert_tokens_to_ids("<|startoftext|>")  # Initial marker
        self.eot_token_id = self.convert_tokens_to_ids("<|endoftext|>")    # End Marker
        self.clean_fn = get_clean_fn(clean)  # Text cleaning compatible with SimpleTokenizer
        self.reduction_fn = get_reduction_mask_fn(reduction_mask) if reduction_mask else None

    def encode(self, text: str, **kwargs) -> List[int]:
        """ **Process text -> Token ID list** """
        text = self.clean_fn(text)  # First, clean up the text.
        token_ids = super().encode(text, add_special_tokens=False)
        return token_ids
    ...
    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None, **kwargs) -> torch.Tensor:
        """ **Batch Processing Compatible with SimpleTokenizer** """
        if isinstance(texts, str):
            texts = [texts]
        ...
        if self.reduction_fn is not None:
            # **Compatible with SimpleTokenizer's reduction_fn mechanism**
            return self.reduction_fn(
                texts,
                context_length=context_length,
                sot_token_id=self.sot_token_id,
        ...
        # **First, tokenize and process the SOT and EOT**
        all_tokens = [[self.sot_token_id] + self.encode(text) + [self.eot_token_id] for text in texts]
